...
| Change Repository Dream behavior | [Canonical Repository Dream](loops/repository-dream.md), [detailed guide](REPOSITORY_DREAM.md) | Preserve one-shot audit + safe execution, high deletion threshold, exact-revision verification and separate deploy/release authority; update catalog version; run the structural check |
| Change backlog refinement | [Canonical Refinement](loops/backlog-refinement.md) | Preserve issue-only scope, context-budget rules and executor-ready handoff; update its catalog version; run the structural check |
...
| Route commands or inputs | [Entry point](AGENTS.md), [catalog](catalog.yaml), [renderer](scripts/prompts.py) | Keep five command routes and six fields consistent; exercise all render routes |
| Improve ChatGPT GitHub operation | [ChatGPT adapter](CHATGPT_DEEP_RUN.md), [GitHub protocol](GITHUB_WORKFLOW.md) | Put essential Deep Run behavior in its canonical template; keep detailed recipes outside copyable prompts; do not impose a ChatGPT adapter on Codex |
...
| Consolidate/prune a repository or refresh repository memory | [Canonical Repository Dream](loops/repository-dream.md), [detailed guide](REPOSITORY_DREAM.md), target entry/map | Reconstruct dependencies before deletion, preserve tests/evidence, classify uncertainty as KEEP, refresh navigation, simplify validation only with proven coverage equivalence, and verify exact PR/final revisions; use [GitHub protocol](GITHUB_WORKFLOW.md) for publication |
...
The four `loops/*.md` files are the canonical copyable prompts. `catalog.yaml` is JSON-compatible YAML with routes, versions, fields and budgets. `scripts/prompts.py` validates and renders; rendered prompts are transient output, not another source to edit. [REPOSITORY_DREAM.md](REPOSITORY_DREAM.md) is the detailed safety/maintenance guide behind the canonical Repository Dream workflow, not a competing prompt. Keep private run specifications outside this public repository. This library has no generated website or target-product dataset to maintain.

## Verification and side effects

From the repository root: `python3 scripts/prompts.py check`.

Requires Python 3.10+; standard library only, no dependency installation or credentials. This command reads files and prints results; it makes no network/model calls or repository writes. Exit 0 means structural checks passed; errors return nonzero. Coverage: canonical files/placeholders/modes, word budgets, versions, local link existence and five synthetic render routes. It does not prove semantic map freshness, runtime agent behavior, website quality or business impact.
...
Reuse a relevant issue checkpoint when one exists. Otherwise search coherent commit messages for `Agent-Run`, then reconcile HEAD, diff and checks before following `Agent-Next`. No checkpoint-only commits or mirrored backlog. Refinement should normally use durable issue state itself as the handoff; create a separate refinement checkpoint only when a large incomplete pass genuinely needs a cursor.
