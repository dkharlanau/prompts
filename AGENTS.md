...
Three user commands, two canonical templates. Read this file explicitly when working through ChatGPT repository tools; do not assume automatic loading. For this library's source paths, coupling, checks and resume route, read [REPO_MAP.md](REPO_MAP.md).

| User intent | Command | Canonical template |
|---|---|---|
| Analyze, fill, clean or prioritize work | Deep Run BACKLOG | [Deep Run](loops/deep-run.md), BACKLOG |
...
Default ordinary “deep loop / improve” requests to EXECUTE; explicit backlog-only requests to BACKLOG. No standalone Backlog Fill/Builder, automatic workflow chaining or prompt copies by project, model or runtime. A deterministic small edit needs no full loop.
...
Run `python3 scripts/prompts.py check`; maintain local links, map routes, catalog/front-matter versions and existing word budgets. Keep copyable templates self-contained. Keep Backlog Executor's runtime contract separate from ChatGPT-specific changes. Record source rationale in [RESEARCH.md](RESEARCH.md).
