...
Five user commands, four canonical templates. Read this file explicitly when working through ChatGPT repository tools; do not assume automatic loading. For this library's source paths, coupling, checks and resume route, read [REPO_MAP.md](REPO_MAP.md).
...
| Audit and safely consolidate repository structure in one run | Repository Dream | [Repository Dream](loops/repository-dream.md) |
...
Default ordinary “deep loop / improve” requests to EXECUTE; explicit backlog-generation requests to BACKLOG; repository consolidation/cleanup/repository-memory/validation-orchestration requests to Repository Dream; explicit queue cleanup/refinement/readiness requests to Backlog Refinement; sustained implementation of an already usable queue to Backlog Executor. Repository Dream is a one-shot AUDIT -> SAFE EXECUTE workflow by default; an explicit AUDIT/read-only/no-writes instruction restricts it to findings. Refinement is intentionally different from BACKLOG: BACKLOG discovers or rewrites justified work around a goal, while Refinement aggressively reconciles, deduplicates, closes, splits/merges and packages an existing queue for cheap execution. No standalone Backlog Fill/Builder, automatic workflow chaining or prompt copies by project, model or runtime. A deterministic small edit needs no full loop.

Automatic adapters are inferred from the actual work, not from magic words in the request. If any selected workflow or direct edit will create or substantially rewrite publishable prose—articles, guides, explainers, knowledge pages, comparisons, landing-page narrative, long-form posts or similar—load [WRITING.md](WRITING.md) before terminology research or drafting. The user never has to say “use the writing prompt.” This applies inside Deep Run EXECUTE, Repository Dream when it materially rewrites publishable prose, Backlog Executor and deterministic content edits. BACKLOG/Refinement should not draft content merely to invoke it; when planned work is content, carry the relevant terminology, reader-value and writing-quality requirements into the issue/handoff.
...
For repository preparation, use [REPOSITORY_SETUP.md](REPOSITORY_SETUP.md). Normal feature work consumes the existing entry/map; setup is not a mandatory prelude to every task. For repository consolidation, dead-file review, pipeline simplification, safe pruning, validation-orchestration cleanup or repository-memory/map refresh, use the canonical [Repository Dream workflow](loops/repository-dream.md) and load [REPOSITORY_DREAM.md](REPOSITORY_DREAM.md) only when its detailed safety guidance is relevant. Treat deletion authority separately and keep uncertain candidates. For model/runtime selection, consult [MODEL_PROFILES.md](MODEL_PROFILES.md). Do not prepend the entire library to a run.

## Maintaining this library

Run `python3 scripts/prompts.py check`; maintain local links, map routes, catalog/front-matter versions and existing word budgets. Keep copyable templates self-contained. Keep Backlog Executor's runtime contract, Backlog Refinement's issue-only contract and Repository Dream's maintenance/integration contract separate from ChatGPT-specific Deep Run changes. Record source rationale in [RESEARCH.md](RESEARCH.md).
